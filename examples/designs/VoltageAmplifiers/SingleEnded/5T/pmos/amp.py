...
class Circuit:
    """5-transistor single-ended OTA.

    PMOS differential pair with NMOS current-mirror active load.
    PMOS tail current source with external VBP bias.
    Single-stage -- no compensation needed.
    """
    ...
    MOSFETS: ClassVar[list[Instance]] = [
        ...
        Instance("M2b", "nmos", d="VOUT", g="n01", s="gnd", b="gnd"),
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        for _ in range(self.fixed_point_iterations):
            ...
            M3_VSB = 0.0
            M2b_VDS = VIN_CM + M1a_VGS - M1b_VDS
            M2b_VSB = 0.0
            Mvbp_VSB = 0.0
        ...
        VOUT_MAX = min(
            M1a_VGS - M1b_VDSAT + VIN_CM,
            -M1b_VDSAT + M1b_VGS + VIN_CM,
            M1a_VGS - M1b_VDSAT - M3_VGS - VBP + VDD + VIN_CM,
            -M1b_VDSAT + M1b_VGS - M3_VGS - VBP + VDD + VIN_CM,
        )
        VOUT_MIN = max(M2b_VDSAT, M2b_VDSAT - M3_VGS - VBP + VDD)
        VIN_MAX = -M1a_VGS - M3_VDSAT + VDD
        VIN_MIN = max(
            M1a_VDSAT - M1a_VGS + M2a_VGS,
            M1a_VDSAT - M1a_VGS + M2b_VGS,
            -M1a_VGS + M1b_VDSAT + VOUT_DC,
            M1a_VDSAT - M1a_VGS + M2a_VGS - M3_VGS - VBP + VDD,
            M1a_VDSAT - M1a_VGS + M2b_VGS - M3_VGS - VBP + VDD,
            -M1a_VGS + M1b_VDSAT - M3_VGS - VBP + VDD + VOUT_DC,
        )
        Output_Swing = VOUT_MAX - VOUT_MIN
        M3_Sat_Margin = M3_VDS - M3_VDSAT
