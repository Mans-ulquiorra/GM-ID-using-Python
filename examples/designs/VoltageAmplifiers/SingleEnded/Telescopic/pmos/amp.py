...
class Circuit:
    """Telescopic-cascode single-ended OTA.

    PMOS diff pair with PMOS/NMOS cascode stack.
    Single-stage -- highest Rout, limited output swing.
    """
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        VOUT_MAX = min(
            -M2b_VDSAT + M2b_VGS + VCASCP, -M2b_VDSAT + M2b_VGS - M5_VGS - VBP + VCASCP + VDD
        )
        VOUT_MIN = max(
            M3b_VDSAT - M3b_VGS + VCASCN, M3b_VDSAT - M3b_VGS - M5_VGS - VBP + VCASCN + VDD
        )
        VIN_MAX = -M1a_VGS - M5_VDSAT + VDD
        VIN_MIN = max(
            M1a_VDSAT - M1a_VGS + M2a_VGS + VCASCP,
            -M1a_VGS + M1b_VDSAT + M2b_VGS + VCASCP,
            M1a_VDSAT - M1a_VGS + M2a_VGS - M5_VGS - VBP + VCASCP + VDD,
            -M1a_VGS + M1b_VDSAT + M2b_VGS - M5_VGS - VBP + VCASCP + VDD,
        )
