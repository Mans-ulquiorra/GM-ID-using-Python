...
class Circuit:
    ...
    NMOS diff pair with PMOS active load (stage 1)
    + NMOS common-source (stage 2).
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        for _ in range(self.fixed_point_iterations):
            ...
            # M4 -- PMOS (second stage, gate driven by first stage = M2b drain)
            M4_GMID = M2a_GMID
            M4_ID = M5_ID
            ...
            # Mvbn -- NMOS (bias diode, independent width)
            Mvbn_W = M3_W * Mvbn_mult
            Mvbn_VGS = M3_VGS
            Mvbn_L = M3_L
            Mvbn_JD = M3_JD
        ...
        # DC balance: M4 must source exactly what M5 sinks at VOUT_DC
        exprs_p = [self.pmos.vsg_expression, self.pmos.vdsat_expression,
                    ...
                    self.pmos.current_density_expression]
        *_, M4_JD_dc = (scalar(x) for x in self.pmos.interpolate(
            length=M4_L, gmid=M2a_GMID, vds=-abs(VDD - VOUT_DC), vbs=0.0, expression=exprs_p))
        exprs_n = [self.nmos.vgs_expression, self.nmos.vdsat_expression,
                    ...
                    self.nmos.current_density_expression]
        *_, M5_JD_dc = (scalar(x) for x in self.nmos.interpolate(
            length=M5_L, gmid=M3_GMID, vds=abs(VOUT_DC), vbs=0.0, expression=exprs_n))
