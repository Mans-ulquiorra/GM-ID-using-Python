...
class Circuit:
    ...
    PMOS diff pair with NMOS active load (stage 1)
    + PMOS common-source (stage 2).
    Miller capacitor + nulling resistor for stability.
    """

    MOSFETS: ClassVar[list[Instance]] = [
        Instance("M1a", "pmos", d="n01", g="VINN", s="n02", b="vdd"),
        Instance("M1b", "pmos", d="n03", g="VINP", s="n02", b="vdd"),
        Instance("M2a", "nmos", d="n01", g="n01", s="gnd", b="gnd"),
        Instance("M2b", "nmos", d="n03", g="n01", s="gnd", b="gnd"),
        Instance("M3", "pmos", d="n02", g="vbp", s="vdd", b="vdd"),
        Instance("M4", "nmos", d="VOUT", g="n03", s="gnd", b="gnd"),
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        for _ in range(self.fixed_point_iterations):
            ...
            # M4 -- NMOS (second stage, gate driven by first stage = M2b_VDS)
            # M4 gets M2a/M2b's VGS circuit-determined, so gm/ID matches M2a
            M4_GMID = M2a_GMID
            M4_ID = M5_ID
            ...
            # M5 -- PMOS (current mirror) -- account for CLM from VDS mismatch
            ...
            # Mvbp -- PMOS (bias diode, independent of M3 width)
            Mvbp_W = M3_W * Mvbp_mult
            Mvbp_VGS = M3_VGS
            Mvbp_L = M3_L
            Mvbp_JD = M3_JD
            ...
            M1a_VDS = VIN_CM + M1a_VGS - M2a_VGS
            M1a_VSB = 0.0
            M1b_VDS = VIN_CM + M1a_VGS - M4_VGS
            M1b_VSB = 0.0
            M2a_VDS = M2a_VGS
            M2a_VSB = 0.0
            M3_VDS = VDD - VIN_CM - M1a_VGS
            M3_VSB = 0.0
            M4_VDS = VOUT_DC
            M4_VSB = 0.0
            M2b_VDS = M4_VGS
        ...
        ss_params = {
            ...
            "M3": {
                ...
                "cdd": M3_cdd * (M3_W / pmos_tw),
            },
            "M4": {
                "gm": M4_GMID * M4_ID,
                "gmb": 0.0,
                "gds": M4_gdsid * M4_ID,
                "cgs": M4_cgs * (M4_W / nmos_tw),
                "cgd": M4_cgd * (M4_W / nmos_tw),
                "cdd": M4_cdd * (M4_W / nmos_tw),
            },
            "M5": {
                "gm": M3_GMID * M5_ID,
                "gmb": 0.0,
        ...
        Itotal = M1a_ID + M1b_ID + M5_ID
        VBP = VDD - M3_VGS
        VOUT_MAX = -M5_VDSAT + VDD
        VOUT_MIN = max(M4_VDSAT, -M3_VGS + M4_VDSAT - VBP + VDD, M4_VDSAT - M5_VGS - VBP + VDD)
        VIN_MAX = -M1b_VGS - M3_VDSAT + VDD
        VIN_MIN = max(
            M1a_VDSAT - M1b_VGS + M2a_VGS,
            M1a_VDSAT - M1b_VGS + M2b_VGS,
            M1b_VDSAT - M1b_VGS + M4_VGS,
            M1a_VDSAT - M1b_VGS + M2a_VGS - M3_VGS - VBP + VDD,
            M1a_VDSAT - M1b_VGS + M2a_VGS - M5_VGS - VBP + VDD,
            M1a_VDSAT - M1b_VGS + M2b_VGS - M3_VGS - VBP + VDD,
            M1a_VDSAT - M1b_VGS + M2b_VGS - M5_VGS - VBP + VDD,
            M1b_VDSAT - M1b_VGS - M3_VGS + M4_VGS - VBP + VDD,
            M1b_VDSAT - M1b_VGS + M4_VGS - M5_VGS - VBP + VDD,
        )
        Output_Swing = VOUT_MAX - VOUT_MIN

        # DC balance: M4 must sink exactly what M5 sources at VOUT_DC
        exprs_n = [self.nmos.vgs_expression, self.nmos.vdsat_expression,
                    self._gdsid, self.nmos.cgs_expression,
                    self.nmos.cgd_expression, self.nmos.cdd_expression,
                    self.nmos.current_density_expression]
        *_, M4_JD_dc = (scalar(x) for x in self.nmos.interpolate(
            length=M4_L, gmid=M4_GMID, vds=abs(VOUT_DC), vbs=0.0, expression=exprs_n))
        exprs_p = [self.pmos.vsg_expression, self.pmos.vdsat_expression,
                    self._gdsid, self.pmos.cgs_expression,
                    self.pmos.cgd_expression, self.pmos.cdd_expression,
                    self.pmos.current_density_expression]
        *_, M5_JD_dc = (scalar(x) for x in self.pmos.interpolate(
            length=M5_L, gmid=M3_GMID, vds=-abs(VDD - VOUT_DC), vbs=0.0, expression=exprs_p))
        DC_Error = abs((M5_JD_dc * M5_W) - (M4_JD_dc * M4_W)) / max(abs(M5_JD_dc * M5_W), 1e-12)
