...
class Circuit:
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        VOUT_MAX = min(
            -M3b_VDSAT + M3b_VGS + VCASCP,
            -M2a_VGS - M3b_VDSAT + M3b_VGS - VBP + VCASCP + VDD,
            -M2b_VGS - M3b_VDSAT + M3b_VGS - VBP + VCASCP + VDD,
        )
        VOUT_MIN = max(
            M4b_VDSAT - M4b_VGS + VCASCN,
            -M2a_VGS + M4b_VDSAT - M4b_VGS - VBP + VCASCN + VDD,
            -M2b_VGS + M4b_VDSAT - M4b_VGS - VBP + VCASCN + VDD,
        )
        VIN_MAX = min(
            -M1a_VDSAT + M1a_VGS + M3a_VGS + VCASCP,
            M1a_VGS - M1b_VDSAT + M3b_VGS + VCASCP,
            -M1a_VDSAT + M1a_VGS - M2a_VGS + M3a_VGS - VBP + VCASCP + VDD,
            -M1a_VDSAT + M1a_VGS - M2b_VGS + M3a_VGS - VBP + VCASCP + VDD,
            M1a_VGS - M1b_VDSAT - M2a_VGS + M3b_VGS - VBP + VCASCP + VDD,
            M1a_VGS - M1b_VDSAT - M2b_VGS + M3b_VGS - VBP + VCASCP + VDD,
        )
        VIN_MIN = max(
            M1a_VGS + M6_VDSAT,
            M1a_VGS - M2a_VGS + M6_VDSAT - VBP + VDD,
            M1a_VGS - M2b_VGS + M6_VDSAT - VBP + VDD,
        )
