...
class Circuit:
    ...
    def __init__(self, lookup_table_path, nmos_name, vdd, cout, iref, k,
        ...
        self.nmos = FastMosfet(lut, nmos_name,
    ...
    def _lookup(self, L, GMID, VDS, VSB):
        r = [scalar(x) for x in self.nmos.interpolate(
            length=L, gmid=GMID, vds=abs(VDS), vbs=-abs(VSB),
            expression=[self.nmos.vgs_expression,
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        for _ in range(self.fixed_point_iterations):
            ...
            M4_VGS, M4_VDSAT, M4_gdsid, M4_gmbsid, M4_JD = self._lookup(
                L_M3, GMID_M3, VOUT_DC - M2_VGS, M2_VGS)
            if M4_W == 0.0:
                M4_W = M4_ID / M4_JD
            ...
            M2_VDS = M2_VGS                      # diode
            M3_VDS = M3_VGS                      # diode
            M4_VDS = VOUT_DC - M2_VGS            # VOUT - V(source)
            M1_VDS = M2_VGS - M3_VGS + M4_VGS    # moseq: n01 = n02 - VGS3 + VGS4
...
def run(*, lookup_table, nmos_name, vdd, cout, iref, k, parameters,
        target_specs, output_module, maxiter=200, seed=1, n_restarts=1,
        fixed_point_iterations=5):
    circuit = Circuit(lookup_table, nmos_name, vdd, cout, iref, k,
