...
class Circuit:
    ...
    Decouples gate and drain of reference devices so the bottom
    pair sits at VDSAT+margin. Better compliance than standard cascode.
    Requires external VBN.
    """
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        for _ in range(self.fixed_point_iterations):
            # M1 -- bottom transistor, input side (ID = IREF)
            ...
            # M2 -- bottom transistor, output side (ID = K * IREF)
    ...
    def compute_vsource_params(self, opt_params: dict[str, float]) -> dict[str, float]:
        ...
        VBN = ops.get("M4", {}).get("VGS", 0.0) + ops.get("M1", {}).get("VDS", 0.0)
        return {"VBN": VBN}
