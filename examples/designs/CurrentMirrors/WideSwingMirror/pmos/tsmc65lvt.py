...
This is the file you normally run and edit.  The sibling mirror.py
...
Wide-Swing Cascode PMOS Current Mirror topology:
  M1, M2 : top mirror pair (gates tied to IREF, sources to vdd)
  M3, M4 : bottom cascode pair (gates tied to VBP)
  M1+M4 : input branch  (carry IREF, current sinks from vdd to IREF)
  M2+M3 : output branch (carry K * IREF, current sources from vdd to VOUT)
...
- Available keys: "Rout", "Vcompliance", "Iout", "Area",
  "Itotal", "VBP".
- To add a new spec, first compute and return it in mirror.py,
...
VDSAT_MARGIN = 0.1  # VDSAT safety margin for M1/M2 mirror devices
...
# GMID_M1 : gm/ID for mirror pair M1, M2
# L_M1    : channel length for mirror pair
...
# Vcompliance is the minimum headroom from vdd needed by the output stack.
TARGET_SPECS = {
