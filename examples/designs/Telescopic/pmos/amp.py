...
class Circuit:
    ...
    MOSFETS: ClassVar[list[Instance]] = [
        ...
        Instance("M2a", "M2a", d="n04", g="vdd", s="n01", b="vdd"),
        Instance("M2b", "M2b", d="VOUT", g="vdd", s="n03", b="vdd"),
        Instance("M3a", "M3a", d="n04", g="gnd", s="n05", b="gnd"),
        Instance("M3b", "M3b", d="VOUT", g="gnd", s="n06", b="gnd"),
        Instance("M4a", "M4a", d="n05", g="n04", s="gnd", b="gnd"),
        Instance("M4b", "M4b", d="n06", g="n04", s="gnd", b="gnd"),
        Instance("M5", "M5", d="n02", g="vdd", s="vdd", b="vdd"),
    ]
    ...
    NETLIST_MOSFETS: ClassVar[list[Instance]] = [
        Instance("M1a", "M1a", d="n01", g="VINP", s="n02", b="vdd"),
        Instance("M1b", "M1b", d="n03", g="VINN", s="n02", b="vdd"),
        Instance("M2a", "M2a", d="n04", g="vcascp", s="n01", b="vdd"),
        Instance("M2b", "M2b", d="VOUT", g="vcascp", s="n03", b="vdd"),
        Instance("M3a", "M3a", d="n04", g="vcascn", s="n05", b="gnd"),
        Instance("M3b", "M3b", d="VOUT", g="vcascn", s="n06", b="gnd"),
        Instance("M4a", "M4a", d="n05", g="n04", s="gnd", b="gnd"),
        Instance("M4b", "M4b", d="n06", g="n04", s="gnd", b="gnd"),
        Instance("M5", "M5", d="n02", g="vbp", s="vdd", b="vdd"),
    ]
    ...
    def __init__(
        ...
        self.device_map: dict[str, dict[str, Any]] = {
            "M1a": {"transistor": self.pmos, "model": pmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M1b": {"transistor": self.pmos, "model": pmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M2a": {"transistor": self.pmos, "model": pmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M2b": {"transistor": self.pmos, "model": pmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M3a": {"transistor": self.nmos, "model": nmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M3b": {"transistor": self.nmos, "model": nmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M4a": {"transistor": self.nmos, "model": nmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M4b": {"transistor": self.nmos, "model": nmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "M5": {"transistor": self.pmos, "model": pmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
            "Mvbp": {"transistor": self.pmos, "model": pmos_name, "length": None, "gmid": None, "current": 0.0, "instances": 1},
        }
    ...
    def evaluate_specs(self, **params: float) -> dict[str, float | None]:
        ...
        for _ in range(self.fixed_point_iterations):
            ...
            # M1a -- PMOS
            (M1a_VGS, M1a_VDSAT, M1a_gm, M1a_gdsid, M1a_cgs, M1a_cgd, M1a_cdd, M1a_JD, M1a_early) = [
            ...
            M1b_L = M1a_L
            M1b_ID = M1a_ID
            (M1b_VGS, M1b_VDSAT, M1b_gm, M1b_gdsid, M1b_cgs, M1b_cgd, M1b_cdd, M1b_JD, M1b_early) = [
                scalar(x) for x in self.pmos.interpolate(
            ...
            # M2a -- PMOS
            M2a_ID = M1a_ID
            (M2a_VGS, M2a_VDSAT, M2a_gm, M2a_gdsid, M2a_cgs, M2a_cgd, M2a_cdd, M2a_JD, M2a_early) = [
                scalar(x) for x in self.pmos.interpolate(
            ...
            # M3a -- NMOS
            M3a_ID = M1a_ID
            (M3a_VGS, M3a_VDSAT, M3a_gm, M3a_gdsid, M3a_cgs, M3a_cgd, M3a_cdd, M3a_JD, M3a_early) = [
            ...
            # M4a -- NMOS
            M4a_ID = M1a_ID
            (M4a_VGS, M4a_VDSAT, M4a_gm, M4a_gdsid, M4a_cgs, M4a_cgd, M4a_cdd, M4a_JD, M4a_early) = [
                scalar(x) for x in self.nmos.interpolate(
            ...
            (M5_VGS, M5_VDSAT, M5_gm, M5_gdsid, M5_cgs, M5_cgd, M5_cdd, M5_JD, M5_early) = [
                scalar(x) for x in self.pmos.interpolate(
                    length=M5_L, gmid=M5_GMID,
                    vds=-abs(M5_VDS), vbs=-abs(M5_VSB),
                    expression=[
                    self.pmos.vsg_expression,
                    self.pmos.vdsat_expression,
                    self.pmos.gm_expression,
            ...
            M2b_L = M2a_L
            M2b_ID = M2a_ID
            (M2b_VGS, M2b_VDSAT, M2b_gm, M2b_gdsid, M2b_cgs, M2b_cgd, M2b_cdd, M2b_JD, M2b_early) = [
                scalar(x) for x in self.pmos.interpolate(
            ...
            M3b_L = M3a_L
            M3b_ID = M3a_ID
            (M3b_VGS, M3b_VDSAT, M3b_gm, M3b_gdsid, M3b_cgs, M3b_cgd, M3b_cdd, M3b_JD, M3b_early) = [
                scalar(x) for x in self.nmos.interpolate(
            ...
            M4b_L = M4a_L
            M4b_ID = M4a_ID
            (M4b_VGS, M4b_VDSAT, M4b_gm, M4b_gdsid, M4b_cgs, M4b_cgd, M4b_cdd, M4b_JD, M4b_early) = [
                scalar(x) for x in self.nmos.interpolate(
            ...
            M1b_VSB = 0.0
            M2a_VDS = VIN_CM - M1a_VDS + M1a_VGS - M4a_VGS
            M2a_VSB = abs(VDD - VIN_CM + M1a_VDS - M1a_VGS)
            M3a_VDS = -M4a_VDS + M4a_VGS
            M3a_VSB = abs(M4a_VDS)
            M4a_VDS = M4a_VDSAT + M4a_VDSAT_MARGIN
            M4a_VSB = 0.0
            M5_VDS = VDD - VIN_CM - M1a_VGS
            M5_VSB = 0.0
            M2b_VDS = M2a_VDS
            M2b_VSB = abs(VDD - VIN_CM + M1a_VDS - M1a_VGS + M2a_VGS - M2b_VGS)
            M3b_VDS = VIN_CM - M1a_VDS + M1a_VGS - M2a_VGS - M2b_VDS + M2b_VGS - M3a_VGS + M3b_VGS - M4a_VDS
        ...
        self.device_dimensions = {
            ...
            "M5": {"Length": M5_L, "Width": M5_W, "Area": M5_L * M5_W, "Current": M5_ID, "GMID": M5_GMID},
            "Mvbp": {"Length": Mvbp_L, "Width": Mvbp_W, "Area": Mvbp_L * Mvbp_W, "Current": Mvbp_ID, "GMID": M5_GMID},
        }
        ...
        ss_params = {
            ...
            "M5": {
                ...
                "gmb": 0.0,
                "gds": M5_gdsid * M5_ID,
                "cgs": M5_cgs * (M5_W / pmos_tw),
                "cgd": M5_cgd * (M5_W / pmos_tw),
                "cdd": M5_cdd * (M5_W / pmos_tw),
            },
        }
        self.passive_params = {"COUT": COUT}
        ss  = build_ss_model(self.MOSFETS, self.PASSIVES, self.VSOURCES, ss_params, self.passive_params)
        ac  = ss.solve(inp='VINP', inn='VINN', out='VOUT',
                       compute_cmrr=True, compute_gbw=True, compute_phase_margin=True)

        # --- Output variables ---
        Area = M1a_L*M1a_W + M1b_L*M1b_W + M2a_L*M2a_W + M2b_L*M2b_W + M3a_L*M3a_W + M3b_L*M3b_W + M4a_L*M4a_W + M4b_L*M4b_W + M5_L*M5_W + Mvbp_L*Mvbp_W
        Itotal = M1a_ID + M1b_ID
        VCASCP = VIN_CM - M1a_VDS + M1a_VGS - M2a_VGS
        VCASCN = M3a_VGS + M4a_VDS
        VBP = VDD - M5_VGS
        VINP = VIN_CM
        VINN = VIN_CM
        VOUT_DC = VIN_CM - M1a_VDS + M1a_VGS - M2a_VGS - M2b_VDS + M2b_VGS
        VOUT_MAX = min(-M2b_VDSAT + M2b_VGS + VCASCP, -M2b_VDSAT + M2b_VGS - M5_VGS - VBP + VCASCP + VDD)
        VOUT_MIN = max(M3b_VDSAT - M3b_VGS + VCASCN, M3b_VDSAT - M3b_VGS - M5_VGS - VBP + VCASCN + VDD)
        VIN_MAX = -M1a_VGS - M5_VDSAT + VDD
        VIN_MIN = max(M1a_VDSAT - M1a_VGS + M2a_VGS + VCASCP, -M1a_VGS + M1b_VDSAT + M2b_VGS + VCASCP, M1a_VDSAT - M1a_VGS + M2a_VGS - M5_VGS - VBP + VCASCP + VDD, -M1a_VGS + M1b_VDSAT + M2b_VGS - M5_VGS - VBP + VCASCP + VDD)
    ...
    def compute_vsource_params(self, opt_params: dict[str, float]) -> dict[str, float]:
        ...
        COUT = self.cout
        VCASCP = VIN_CM - ops.get("M1a", {}).get("VDS", 0.0) + ops.get("M1a", {}).get("VGS", 0.0) - ops.get("M2a", {}).get("VGS", 0.0)
        VCASCN = ops.get("M3a", {}).get("VGS", 0.0) + ops.get("M4a", {}).get("VDS", 0.0)
        VBP = VDD - ops.get("M5", {}).get("VGS", 0.0)
        return {"VCASCP": VCASCP, "VCASCN": VCASCN, "VBP": VBP}
